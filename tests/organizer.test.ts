...
import type { JobPhase, OrganizeJob, Settings } from '../src/lib/types';

const API_KEY = 'sk-test-key-abcdef1234567890';
...
function makeSeed(miscCount: number): SeedNode[] {
...
type AssignPayload = { categories: Array<{ id: string; title: string; path: string[] }>; bookmarks: Array<{ i: number; title: string; url: string }> };

/** 按 system prompt 分流的脚本化 chat。 */
function scriptedChat(record: Recorded[] = [], hooks: { onAssign?: (payload: AssignPayload, signal?: AbortSignal) => Promise<void> | void; failPropose?: unknown } = {}): ChatFn {
...
function harness(miscCount: number, options: { settings?: Settings; chat?: ChatFn; record?: Recorded[] } = {}) {
  ...
  return { api, organizer, persisted, phases, record };
}
...
describe('Organizer – happy path', () => {
  ...
  it('errors when the scope has no loose bookmarks', async () => {
    ...
    const organizer = new Organizer({ bookmarksApi: api, chat: scriptedChat(), persistJob: async () => {}, settings: settings() });
    await organizer.start();
    expect(organizer.state.job.phase).toBe('error');
